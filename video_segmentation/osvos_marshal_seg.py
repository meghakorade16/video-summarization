...
from Codebase.video_segmentation import osvos_mod
from Codebase.video_segmentation.dataset_mod import Dataset


##
# Function to segment images using video level annotation
# @param image_dir, list of images
# @param annotation_dir, location of annotations
# @param mask_dir, location of the output folder
# @param seq_name, name of the video
# @param data_dir, location of the dataset
# @param iterations, number of training iterations
##
def osvos_segment(image_dir, annotation_dir, mask_dir, seq_name, data_dir, model_name='', iterations=500):
    if model_name == '':
        model_name = seq_name
    ...
    if train_model:
        train_imgs = [os.path.join(image_dir, frame.split('.')[0] + '.' + test_img_ext) + ' ' + annotate_imgs[i] for
                      i, frame in enumerate(annotate_frames)]
    ...
    for img_p in test_frames:
        ...
        cv2.imwrite(os.path.join(result_path_vis, frame_num + '.png'),
                    cv2.cvtColor(im_over.astype(np.uint8), cv2.COLOR_RGB2BGR))
