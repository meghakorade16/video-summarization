# Modifications made by Brent Griffin to work with Marshal framework.
...
def random_pair_shuffle_old(idx, values):
	# Randomize order, but keep pair together.
	# Could improve using difference of values.
	print ('Using modified group index shuffling.')
	groupIdx = 0
	group = [[idx[0]]]
	for i in range(1,len(idx)):
		# Compare current index value with next index.
		if values[idx[i]] == values[idx[i-1]]:
			# If they match, add next index to current group.
			group[groupIdx].append(idx[i])
		else:
			# Else, add to a new group.
			groupIdx += 1
			group.append([idx[i]])
	# Randomize group order, and then rebuild index.
	np.random.shuffle(group)
	idxOut = [];
	for i, groupIdx in enumerate(group):
		for j, idxIdx in enumerate(groupIdx):
			idxOut.append(idxIdx)
	return idxOut
...
class Dataset:
	def __init__(self, train_list, test_list, database_root, store_memory=True, data_aug=False):
		...
		for i in range(0,self.train_size):
			self.values.append(self.images_train[i].shape)
			if self.verbose:
				print ('Image size is ' + str(self.values[i]))
		# self.train_idx = random_pair_shuffle(self.train_idx, self.values)
	...
	def next_batch(self, batch_size, phase):
		...
		"""
		#print '\n\ninside of next_batch in dataset_mod.py\n\n'
		#IPython.embed()
		if phase == 'train':
			...
			#print '\n\ninside dataset next_batch return\n\n'
			#IPython.embed()
